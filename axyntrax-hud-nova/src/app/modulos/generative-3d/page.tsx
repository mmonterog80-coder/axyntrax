...
export default function generative3dModule() {
  const [active, setActive] = useState(false);
  const Icon = (Icons as any)['Box'] || Icons.Box;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold flex items-center gap-3">
            ...
            { 'Generative 3D Weaver' }
          </h1>
          <p className="text-gray-400 mt-2">{ 'Conversión de texto a mallas 3D para comercio y Metaverso.' }</p>
        </div>
        <button 
            ...
            <div className="font-mono text-xs text-green-400/80 space-y-2 h-32 overflow-y-auto">
              ...
              {active && (
                <>
                  ...
                  <p className="text-cyan-400">{'>'} Módulo Generative 3D Weaver ONLINE.</p>
                </>
              )}
            </div>
          </motion.div>
