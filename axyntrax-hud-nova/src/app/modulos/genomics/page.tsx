...
export default function genomicsModule() {
  const [active, setActive] = useState(false);
  const Icon = (Icons as any)['Dna'] || Icons.Box;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold flex items-center gap-3">
            <Icon className="text-indigo-500 w-10 h-10" />
            { 'Genomic Analyst' }
          </h1>
          <p className="text-gray-400 mt-2">{ 'Panel predictivo de salud basado en secuenciación genética.' }</p>
        </div>
        <button 
            ...
            <div className="font-mono text-xs text-green-400/80 space-y-2 h-32 overflow-y-auto">
              ...
              {active && (
                <>
                  ...
                  <p className="text-cyan-400">{'>'} Módulo Genomic Analyst ONLINE.</p>
                </>
              )}
            </div>
          </motion.div>
